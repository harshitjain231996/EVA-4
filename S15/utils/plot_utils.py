...
'''
def visualize_data(im_data, figsize=(24,8), filename=None, show=True, nrow=4):
    try:
      im_data = im_data[:16]
...
def visualize_data_norm(im_data, means, stds, figsize=(24,8), filename=None, show=True, nrow=4):
    im_data = im_data.detach().cpu()
    im_data = (im_data*stds[None, :, None, None]) + means[None, :, None, None]
    visualize_data(im_data, figsize=figsize, filename=filename, show=show, nrow=nrow)
